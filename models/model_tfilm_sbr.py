...
class MBSEANet_film_sbr(nn.Module):
    def __init__(self, min_dim=32, strides=[1,2,2,2], 
    ...
    def forward(self, x, cond):
        ...
        # SBR replaces input conv: [B,C,T] -> [B,C_out,T]
        x = x.repeat(1, self.min_dim//self.c_in + 1, 1)[:,:self.min_dim,:]
        # x = self.conv_in(x)
        skip.append(x)
